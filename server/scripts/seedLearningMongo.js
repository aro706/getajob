...
async function seed() {
  await connectDB();
  console.log("🌱 Clearing and Seeding Learning Resources...");
  
  // Important: Clear old entries to prevent duplicates
  await LearningResource.deleteMany({});

  for (const item of learningData) {
    const vector = await generateEmbedding(item.content);
    await LearningResource.create({ ...item, embedding: vector });
    console.log(`✅ Seeded: ${item.goal}`);
  }
